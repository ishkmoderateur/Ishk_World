...
import { Plus, Edit, Trash2, Coins, ArrowLeft, Search } from "lucide-react";
import Link from "next/link";

export default function CreditBundlesPanel() {
  ...
  const fetchBundles = async () => {
    try {
      setError("");
      const response = await fetch("/api/admin/credit-bundles");
      if (response.ok) {
        const data = await response.json();
        setBundles(Array.isArray(data) ? data : []);
      } else {
        const errorData = await response.json().catch(() => ({ error: "Failed to fetch bundles" }));
        setError(errorData.error || `Error ${response.status}: ${response.statusText}`);
      }
    } catch (error) {
      console.error("Error fetching bundles:", error);
      setError("Failed to fetch bundles. Please check your connection and try again.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent | React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (!formData.name || !formData.slug || formData.credits === "" || formData.price === "") {
      setError("Name, slug, credits, and price are required");
      return;
    }
    ...
    try {
      const url = editing ? `/api/admin/credit-bundles/${editing.id}` : "/api/admin/credit-bundles";
      ...
      if (response.ok) {
        setSuccess(editing ? "Bundle updated successfully!" : "Bundle created successfully!");
        fetchBundles();
        setTimeout(() => {
          resetForm();
          setSuccess("");
        ...
        setError(data.error || `Failed to ${editing ? "update" : "create"} bundle`);
      }
    } catch (error) {
      console.error("Error saving bundle:", error);
      setError("An error occurred while saving the bundle");
    }
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const response = await fetch(`/api/admin/credit-bundles/${id}`, {
        method: "DELETE",
      });
      if (response.ok) {
        fetchBundles();
      ...
      setError("An error occurred while deleting the bundle");
    }
  };
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-charcoal/60 mt-1">Manage credit bundles and packages</p>
              </div>
            </div>
            <button
                ...
                {filteredBundles.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="px-6 py-8 text-center text-charcoal/60">
                      {searchTerm ? "No bundles found" : "No bundles found"}
                    </td>
                  </tr>
                ) : (
                  filteredBundles.map((bundle) => (
                    <tr key={bundle.id} className="hover:bg-sage/5 transition-colors">
                      ...
                      <td className="px-6 py-4 text-sm text-charcoal">
                        {bundle.credits} {bundle.bonusCredits > 0 && `+ ${bundle.bonusCredits} bonus`}
                      </td>
                      <td className="px-6 py-4 text-sm text-charcoal">
                      ...
                      <td className="px-6 py-4 text-sm">
                        <span className={`px-2 py-1 rounded-lg text-xs font-medium ${
                          bundle.isActive ? "bg-sage/10 text-sage" : "bg-gray/10 text-gray"
                        }`}>
