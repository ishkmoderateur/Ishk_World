...
import { Plus, Edit, Trash2, Camera, ArrowLeft, Search } from "lucide-react";
import Link from "next/link";
import PriceInput from "@/components/price-input";

export default function PhotographyServicesPanel() {
  ...
  const handleSubmit = async (e: React.FormEvent | React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (!formData.name || !formData.slug || !formData.description || !formData.duration) {
      setError("Name, slug, description, and duration are required");
      return;
    }

    try {
      const url = editing ? `/api/admin/photography-services/${editing.id}` : "/api/admin/photography-services";
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const response = await fetch(`/api/admin/photography-services/${id}`, {
        method: "DELETE",
      });
      if (response.ok) {
        fetchServices();
  ...
  const handleEdit = (service: any) => {
    ...
    setFormData({
      ...
      comparePrice: service.comparePrice?.toString() || "",
      features: features,
      image: service.image || "",
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-charcoal/60 mt-1">Manage photography services</p>
              </div>
            </div>
            <button
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-4 py-3 rounded-xl border border-sage/20 focus:outline-none focus:ring-2 focus:ring-sage"
                    placeholder="e.g., 2 hours, 4 hours, Full day"
                    required
                  />
                </div>
                <div>
            ...
            <table className="w-full">
              <thead className="bg-sage/5">
                <tr>
                  ...
                  <th className="px-6 py-4 text-left text-sm font-medium text-charcoal">Duration</th>
                ...
                {filteredServices.length === 0 ? (
                  ...
                  filteredServices.map((service) => (
                    <tr key={service.id} className="hover:bg-sage/5 transition-colors">
                      ...
                      <td className="px-6 py-4 text-sm text-charcoal/60">{service.duration}</td>
                      <td className="px-6 py-4 text-sm text-charcoal">
                        {service.price ? `€${service.price.toFixed(2)}` : "Price on request"}
                      </td>
